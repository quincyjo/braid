...
object JsonBean {
  ...
  final val True: JsonBean = JBoolean(true)
  final val False: JsonBean = JBoolean(false)
  final val Null: JsonBean = JNull

  def apply(value: JsonValueWrapper): JsonBean = value.value

  def arr(values: JsonValueWrapper*): JsonBean = JArray(
  ...
  def fromFields(values: Iterable[(String, JsonBean)]): JObject = JObject(
    values.toMap
  )
  ...
  def number(bigDecimal: BigDecimal): JsonBean = JNumber(bigDecimal)
  ...
  implicit object JsonBeanBraid extends Braid[JsonBean] {
    ...
    override def mapObject(
        json: JsonBean
    )(f: Map[String, JsonBean] => Map[String, JsonBean]): JsonBean =
      asObject(json)
        .map(attributes => JsonBean.fromFields(f(attributes)))
        .getOrElse(json)

    override def mapArray(
        json: JsonBean
    )(f: Vector[JsonBean] => Vector[JsonBean]): JsonBean =
      asArray(json)
        .map(values => JsonBean.fromValues(f(values)))
        .getOrElse(json)

    override def mapString(
        json: JsonBean
    )(f: String => String): JsonBean =
      asString(json)
        .map(value => JString(f(value)))
        .getOrElse(json)

    override def mapBoolean(
        json: JsonBean
    )(f: Boolean => Boolean): JsonBean =
      asBoolean(json)
